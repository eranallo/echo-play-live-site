...
function TaskPreview({ task }) {
  return <a className="cv-task" href="/admin/tasks"><span>{task.status}</span><strong>{task.title}</strong><p>{task.dueLabel} · {task.priority}</p></a>
}

function ApprovalPreview({ item }) {
  return <a className="cv-task" href="/admin/approvals"><span>{item.status}</span><strong>{item.item}</strong><p>{item.riskLevel} · {item.createdAt}</p></a>
}

function SpecialistShortcut({ guide }) {
  return <a className="cv-specialist" href={`/admin/specialists/${guide.kind}`}><span>{guide.group}</span><strong>{guide.shortLabel}</strong><p>{guide.tagline}</p></a>
}

function TableStatus({ table }) {
  return <div className={`cv-table ${table.ready ? 'cv-table-ready' : ''}`}><span>{table.ready ? 'Ready' : table.missing ? 'Missing' : 'Issue'}</span><strong>{table.label}</strong><p>{table.records} records</p></div>
}
      ...
      <div className="cv-wrap">
        ...
        <header className="cv-hero">
          <div>
            ...
            <h1>Run the day.</h1>
            <p>Your command center should show the work, not make you hunt for it. Open quick panels here when you only need context; jump pages only when you need deeper action.</p>
        ...
        <section className="cv-quick" aria-label="Command center quick panels">
          <details className="cv-drawer" open>
            <summary>Today quick view <span>{queue.length} items</span></summary>
            <div className="cv-drawer-body"><div className="cv-queue-list">{queue.length ? queue.slice(0, 5).map((item, index) => <QueueItem key={`${item.type}-${item.title}-${index}`} item={item} index={index} />) : <div className="cv-empty">No urgent items loaded.</div>}</div></div>
          </details>
          <details className="cv-drawer">
            <summary>Specialist shortcuts <span>{specialists.length} homepages</span></summary>
            <div className="cv-drawer-body"><div className="cv-mini-grid">{specialists.map(guide => <SpecialistShortcut key={guide.kind} guide={guide} />)}</div></div>
          </details>
          <details className="cv-drawer">
            <summary>Approvals + tasks <span>{pendingApprovals.length + openTasks.length} open</span></summary>
            <div className="cv-drawer-body"><div className="cv-mini-grid">{pendingApprovals.slice(0, 4).map(item => <ApprovalPreview key={item.id} item={item} />)}{openTasks.slice(0, 4).map(task => <TaskPreview key={task.id} task={task} />)}{!pendingApprovals.length && !openTasks.length && <div className="cv-empty">No open approvals or Airtable tasks loaded.</div>}</div></div>
          </details>
          <details className="cv-drawer">
            <summary>System readiness <span>{tables.filter(table => table.ready).length}/{tables.length} ready</span></summary>
            <div className="cv-drawer-body"><div className="cv-mini-grid">{tables.map(table => <TableStatus key={table.label} table={table} />)}</div></div>
          </details>
        </section>
