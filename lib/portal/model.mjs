...
export function namesForLinkedRecords(value, recordMap, getRecordName) {
  if (!recordMap || typeof recordMap.get !== 'function' || typeof getRecordName !== 'function') {
    return []
  }

  return linkedIds(value)
    .map(id => getRecordName(recordMap.get(id)))
    .filter(Boolean)
}
