...
import { partitionShowsByDate } from '@/lib/portal/showNavigation.mjs'
...
function linkedNames(ids, map, nameFields) {
  return namesForLinkedRecords(ids, map, record => getName(record, nameFields))
}
...
function normalizeShow(record, maps = {}) {
  ...
  return {
    ...
    lifecycle: selectName(fields['Lifecycle Stage'] || fields.Status, 'Inquiry'),
    venueId: venueIds[0] || '',
...
export async function getPortalDirectory() {
  try {
    ...
    const showYears = [...new Set(data.shows
      .map(show => String(show.fields?.Date || '').slice(0, 4))
    ...
    return {
      ...
      counts: { upcomingShows, allShows: data.shows.length, showYears },
    }
  } catch (error) {
    return {
      ok: false,
      error: safePortalError(error),
      members: [],
      crew: [],
      counts: { upcomingShows: 0, allShows: 0, showYears: [] },
    }
  }
}
...
export async function getPortalCalendar() {
  try {
    const [showRecords, venueRecords, bandRecords] = await Promise.all([
      fetchRecords(TABLES.shows),
      fetchRecords(TABLES.venues),
      fetchRecords(TABLES.bands),
    ])
    const maps = {
      venues: mapById(venueRecords),
      bands: mapById(bandRecords),
    }
    const shows = showRecords.map(record => normalizeShow(record, maps))
    const partitioned = partitionShowsByDate(shows)

    return {
      ok: true,
      error: null,
      shows: [...partitioned.upcoming, ...partitioned.previous],
      counts: {
        all: shows.length,
        upcoming: partitioned.upcoming.length,
        previous: partitioned.previous.length,
        undated: shows.filter(show => !show.date).length,
      },
    }
  } catch (error) {
    return {
      ok: false,
      error: safePortalError(error, 'Master calendar failed to load.'),
      shows: [],
      counts: { all: 0, upcoming: 0, previous: 0, undated: 0 },
    }
  }
}
...
export async function getMemberPortal(memberId) {
  try {
    ...
    const assignedRecords = data.shows.filter(show => personIsAssigned(show, memberId, 'member'))
    const assignmentGroups = partitionShowsByDate(assignedRecords)
    const assigned = assignmentGroups.upcoming
      .map(show => assignmentForShow(show, memberRecord, 'member', data))
    const previousShows = assignmentGroups.previous
      .map(show => assignmentForShow(show, memberRecord, 'member', data))
...
export async function getCrewPortal(crewId) {
  try {
    ...
    const assignedRecords = data.shows.filter(show => personIsAssigned(show, crewId, 'crew'))
    const assignmentGroups = partitionShowsByDate(assignedRecords)
    const assigned = assignmentGroups.upcoming
