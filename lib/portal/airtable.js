...
function assertToken() {
  if (!AIRTABLE_API_TOKEN) {
    throw new Error('Missing Airtable server credential.')
  }
}
...
async function airtableFetch(url) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Airtable portal request failed (${response.status}): ${text.slice(0, 200)}`)
  }
...
export async function getPortalDirectory() {
  try {
    ...
    return { ok: false, error: error?.message || 'Portal directory failed to load.', members: [], crew: [], counts: { upcomingShows: 0 } }
  }
}
...
export async function getMemberPortal(memberId) {
  try {
    ...
    return { ok: false, error: error?.message || 'Member portal failed to load.', person: null, shows: [], blackouts: [] }
  }
}
...
export async function getCrewPortal(crewId) {
  try {
    ...
    return { ok: false, error: error?.message || 'Crew portal failed to load.', person: null, shows: [], blackouts: [] }
  }
}
...
export async function getPortalShow(showId) {
  try {
    ...
    return { ok: false, error: error?.message || 'Portal show failed to load.', show: null, setlist: null }
  }
}
