...
import { formatPortalTime, normalizeRunOfShowSegments } from '@/lib/portal/runOfShow.mjs'
...
function normalizeShow(record, maps = {}) {
  ...
  return {
    ...
    venueName: venue ? getName(venue, ['Venue Name', 'Name']) : 'Venue TBD',
    venueAddress: f['Venue Address'] || venue?.fields?.Address || '',
    venueCity: venue?.fields?.City || '',
    venueState: venue?.fields?.State || '',
    trailerLoadIn: formatPortalTime(f['Trailer Load-In Time']),
    loadIn: formatPortalTime(f['Load-In Time']),
    soundCheck: formatPortalTime(f['Sound Check Time']),
    start: formatPortalTime(f['Start Time']),
    end: formatPortalTime(f['End Time']),
...
async function fetchShowSegments(showRecord) {
  const segmentIds = linkedIds(showRecord?.fields?.['SHOW SEGMENTS'])
  if (!segmentIds.length) return []

  const records = await Promise.all(
    segmentIds.map(id => fetchRecord(TABLES.showSegments, id).catch(() => null))
  )

  return normalizeRunOfShowSegments(records.filter(Boolean))
}
...
export async function getPortalRunOfShow(showId) {
  try {
    const data = await getBasePortalData()
    const showRecord = data.maps.shows.get(showId) || await fetchRecord(TABLES.shows, showId)
    const segments = await fetchShowSegments(showRecord)
