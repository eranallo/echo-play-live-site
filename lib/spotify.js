...
// Phase 20.5 hotfix: bumped v2→v3 + added retry-on-null wrapper. Same logic
// as searchArtist above. If the cache has a stale null (poisoned by the
// earlier limit=50 issue), the wrapper transparently retries uncached so
// production never serves empty discography just because of a one-off failure.
const _cachedDiscography = unstable_cache(
  async (artistId) => _getArtistDiscographyOnce(artistId),
  ['spotify-discography-v3'],
