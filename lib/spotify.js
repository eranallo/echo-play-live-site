...
const _cachedSearchArtist = unstable_cache(
  ...
  ['spotify-artist-v1'],
  { revalidate: 60 * 60 * 24 * 90, tags: ['spotify-artists'] } // 90 days
)
...
async function _getArtistDiscographyOnce(artistId) {
  ...
  // 1. Get the list of albums (album_type=album to skip singles/compilations).
  //    50 per page is the max; most artists have well under that.
  const albumIds = []
  const albumsMeta = []
  let pageUrl = `${ARTIST_ALBUMS_URL(artistId)}?include_groups=album&limit=50&market=US`
  try {
    while (pageUrl) {
...
const _cachedDiscography = unstable_cache(
  ...
  ['spotify-discography-v1'],
  { revalidate: 60 * 60 * 24 * 30, tags: ['spotify-discography'] } // 30 days
)
