...
// Phase 20.5 hotfix: bumped v2→v3 because v2 picked up null entries during
// the limit=50→20 transition. Also added retry-on-null below so a poisoned
// cache can't take production down indefinitely — production heals as soon
// as the next request succeeds.
...
export async function searchArtist(name) {
  ...
  const cached = await _cachedSearchArtist(name)
  if (cached) return cached
  // Cache returned null — for canonical artists like "Deftones" or "TOOL"
  // this is never a legitimate result, so try once uncached. The fresh result
  // isn't written back to the cache from this branch (unstable_cache only
  // writes when its wrapped function runs through the cache layer itself).
  return _searchArtistOnce(name)
}

// Normalize an album name for dedup purposes (Phase 20.8). Strips parenthetical
// suffixes ("(Deluxe Edition)", "(Remastered)", etc.) and dash-separated reissue
// keywords ("- Remastered", "- 20th Anniversary"). Goal: "White Pony" and
// "White Pony (20th Anniversary Deluxe Edition)" collapse to the same key.
function normalizeAlbumName(name) {
  return (name || '')
    .toLowerCase()
    .replace(/\s*\([^)]*\)\s*$/, '')                                       // trailing (parenthetical)
    .replace(/\s*[-–—:]\s*(remaster|deluxe|anniversary|special|expanded|reissue|bonus|legacy|edition)[^|]*$/i, '')
    .trim()
}
...
async function _getArtistDiscographyOnce(artistId) {
  ...
  //    Phase 20.6 hotfix: dropped page size 20→10. Spotify's docs say the max
  //    is 50, but the live API rejects every value ≥ 20 with HTTP 400
  //    "Invalid limit" on this specific endpoint (confirmed via the
  //    /api/spotify-check probe sweep — values 1, 10, and null worked; 20, 30,
  //    49, 50 all failed). The pagination loop below follows `next`, so deep
  //    catalogs (Deftones has 15 raw albums via Spotify) still resolve fully —
  //    just in 2 requests instead of 1.
  const albumIds = []
  const albumsMeta = []
  let pageUrl = `${ARTIST_ALBUMS_URL(artistId)}?include_groups=album&limit=10&market=US`
  try {
    while (pageUrl) {
  ...
  // 2. Fetch full album details per ID. Phase 20.7 hotfix: we used to batch
  //    via GET /v1/albums?ids=... but Spotify silently lowered or broke that
  //    endpoint the same way they did /v1/artists/{id}/albums (the diagnostic
  //    in 20.5 caught it). Per-album calls are slower per cold start (15
  //    requests for Deftones instead of 1) but every result caches for 7 days
  //    via unstable_cache, so steady state is identical. Promise.allSettled
  //    means a single 4xx on one album doesn't take down the whole list.
  const perAlbumResults = await Promise.allSettled(
    albumIds.map(async (id) => {
      ...
      if (!res.ok) {
        console.warn(`[spotify] album ${id} fetch failed:`, res.status)
        return null
      }
      return res.json()
    })
  )
  const albumDetails = perAlbumResults
    .map(r => (r.status === 'fulfilled' ? r.value : null))
    .filter(Boolean)

  // 3. De-dupe albums by normalized name (no year). Phase 20.8: previously we
  //    keyed on name + year, which kept reissues separate from originals
  //    (e.g., "White Pony" 2000 AND "White Pony (20th Anniversary Deluxe
  //    Edition)" 2020). Now we group by canonical name and keep the EARLIEST
  //    release_date in each group — that's almost always the original studio
  //    release, which is what fans recognize.
  const dedup = new Map()
  for (const a of albumDetails) {
    if (!a) continue
    const key = normalizeAlbumName(a.name)
    if (!key) continue
    ...
    const existingDate = existing.release_date || '9999-99-99'
    const candidateDate = a.release_date || '9999-99-99'
    // ISO date strings sort correctly as strings, so "2000-06-20" < "2020-12-11".
...
// Phase 20.8 hotfix: bumped v5→v6 because v5 entries include deluxe/anniversary
// reissues alongside originals. v6 reflects the new dedup logic that collapses
// reissues into their original album.
const _cachedDiscography = unstable_cache(
  async (artistId) => _getArtistDiscographyOnce(artistId),
  ['spotify-discography-v6'],
...
export async function getArtistDiscography(artistId) {
  ...
  const cached = await _cachedDiscography(artistId)
  if (cached && cached.albums && cached.albums.length > 0) return cached
  // Empty cache hit — try fresh once. Avoids permanent zero-album state when
  // a transient Spotify failure poisons the cache.
  return _getArtistDiscographyOnce(artistId)
}
