...
export function publicShowToEventJsonLd(show, siteUrl = 'https://echoplay.live') {
  ...
  return {
    ...
    description: `${show.bands.map(band => band.name).join(' + ')} ${show.state === 'canceled' ? 'was scheduled to play' : 'plays'} at ${show.venue.name}. ${show.state === 'canceled' ? 'This show has been canceled.' : 'See the lineup, venue and ticket details.'}`,
