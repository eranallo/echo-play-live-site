...
export function prepareShowRows(records, today) {
  ...
  for (const record of records || []) {
    ...
    rows.push({
      ...
      ticketPrice: fields[SHOW_FIELD_IDS.ticketPrice],
    })
  }
...
function normalizedRow(row, bandNamesById, venueNamesById, publicBandsById) {
  ...
  return {
    ...
    row: {
      ...
      priceLabel: priceLabel(row.ticketPrice),
    },
  }
}
...
function buildPublicShow(rows, groupKey) {
  ...
  const price = oneValue(rows, (row) => row.priceLabel)
  const headliner = oneValue(rows, (row) => row.headlinerSlug)

  if (
    ![date, venueId, venueName, status, startTime, ticketUrl, price, headliner].every(
      (item) => item.ok,
    )
  ) {
  ...
  const ticket =
    !canceled && ticketUrl.value
      ? { label: 'Get Tickets', url: ticketUrl.value, priceLabel: price.value }
      : null
  const ticketNote = canceled
    ? null
    : ticket
      ? null
      : price.value
        ? `${price.value} · Ticket details coming soon`
...
export function buildPublicShows({ rows, bandNamesById, venueNamesById, approvedBands }) {
  const reasons = {}
  const validRows = []
  ...
  for (const row of rows || []) {
    const normalized = normalizedRow(row, bandNamesById, venueNamesById, publicBandsById)
    if (!normalized.ok) {
...
export function publicShowToEventJsonLd(show, siteUrl = 'https://echoplay.live') {
  ...
  return {
    ...
    location: { '@type': 'Place', name: show.venue.name },
