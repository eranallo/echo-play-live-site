...
import { beginFile, ensureSubmissionFolder, fileMetadata, filePosition, generateDriveIds, uploadDestinations, verifiedFile, verifyFileSignature } from './google.mjs'
...
export async function runUpload(action, token, body, ip, origin='https://echoplay.live') {
  ...
  let parsed
  if(action==='start') {
    parsed=validateSubmission(body)
  ...
  if(row?.data.status==='complete') return publicSession(row.data)
  ...
  if(action==='complete') {
    ...
    try {
      const verified=await inGroups(row.data.files,async f=>verifiedFile(await fileMetadata(f.id),f,row.data) && await verifyFileSignature(f))
      if(verified.some(ok=>!ok)) throw new Error('delivery_incomplete')
      ...
      row=await writeJournal(path,{...row.data,status:'complete',receiptId,completedAt:new Date().toISOString(),leaseUntil:0,files:row.data.files.map(({uploadUrl,...f})=>f)},row.etag)
      return publicSession(row.data)
    } catch(error) {
