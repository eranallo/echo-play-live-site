...
export function hasMediaSignature(bytes, mimeType) {
  ...
  if(mimeType==='video/webm')return hex.startsWith('1a45dfa3') && ascii.includes('webm')
  ...
  for(let offset=16;offset+4<=Math.min(boxSize,bytes.length);offset+=4)brands.push(ascii.slice(offset,offset+4))
  if(['image/heic','image/heif'].includes(mimeType))return brands.some(b=>/^(heic|heix|hevc|hevx|mif1|msf1)$/.test(b))
  if(mimeType==='image/avif')return brands.some(b=>/^(avif|avis)$/.test(b))
  // Audio-only and still-image brands must not pass as an MP4 video simply
  // because their compatible-brand list also contains the generic ISO brand.
  if(brands.some(b=>/^(M4A |M4B |M4P |F4A |F4B |avif|avis|heic|heix|hevc|hevx|mif1|msf1)$/.test(b)))return false
  return ['video/mp4','video/quicktime','video/x-m4v'].includes(mimeType) && brands.some(b=>/^(isom|iso[2-9]|mp4[12]|avc1|M4V |M4VH|M4VP|qt  |MSNV|3gp[0-9])$/.test(b))
}
