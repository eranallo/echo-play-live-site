import { UPLOAD_CHUNK_BYTES, mediaType, hasMediaSignature } from './files.mjs'
export async function checkSelectedMedia(file) {
  const mime=mediaType(file.name)
  if(!mime || !hasMediaSignature(new Uint8Array(await file.slice(0,64).arrayBuffer()),mime)) {
    throw new Error('Please choose original photos or videos. Documents, audio files, archives and renamed files aren’t supported.')
  }
}
