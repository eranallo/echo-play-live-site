...
export async function readJournal(path, { maxBytes=250000 } = {}) {
  ...
  if (result.statusCode !== 200 || result.blob.size > maxBytes || !result.blob.etag || result.blob.etag.startsWith('W/')) throw new Error('journal_invalid')
...
export async function listUploadEmailQueue() {
  const result=await list({...options(),prefix:'upload-email/pending/',limit:40})
  return result.blobs.map(blob=>blob.pathname)
}
export async function removeUploadEmailQueue(path) {
  if(!/^upload-email\/pending\/[a-f0-9]{64}\.json$/.test(path))throw new Error('invalid_email_path')
