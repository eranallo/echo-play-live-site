...
export async function trashRejectedMedia(file, session, {metadata=fileMetadata,request=driveRequest}={}) {
  // Only the exact file created for this submission is eligible for cleanup.
  // Trash is recoverable and never grants additional Drive access.
  const current=await metadata(file.id)
  if(!current || !verifiedFile({...current,trashed:false},file,session))throw new Error('delivery_mismatch')
  if(current.trashed)return
  const response=await request(`/files/${encodeURIComponent(file.id)}?supportsAllDrives=true&fields=id,trashed`,{method:'PATCH',body:JSON.stringify({trashed:true})})
  if(!response.ok || (await response.json()).trashed!==true)throw new Error('rejected_media_cleanup')
}
