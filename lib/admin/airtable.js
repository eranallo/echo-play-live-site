...
export async function approveSaveEventDescription(approvalId) {
  ...
  await markApprovalStatus(approvalId, 'Approved', `Completed by admin dashboard at ${new Date().toISOString()}.`)

  await createAiRunLog({
    name: 'Approved Campaign Draft',
    status: 'Complete',
    notes: `Saved generated event description to SHOWS.Event Description for show ${showId}. Approval: ${approvalId}`,
  }).catch(() => null)
