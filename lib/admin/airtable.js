...
const TABLES = {
  ...
  merchSales: process.env.AIRTABLE_MERCH_SALES_TABLE || 'MERCH SALES',
}
    ...
    return {
      ...
      show: {
        ...
        editor: {
          ...
          advanceNotes: fields['Advance Notes'] || '',
          eventContactName: fields['Event Contact Name'] || '',
          eventContactPhone: fields['Event Contact Phone'] || '',
          eventContactEmail: fields['Event Contact Email'] || '',
          actualAttendance: fields['Actual Attendance'] ?? '',
          grossTicketRevenue: fields['Gross Ticket Revenue'] ?? '',
          settlementNotes: fields['Settlement Notes'] || '',
          showRecap: fields['Show Recap'] || '',
          lessonsLearned: fields['Lessons Learned'] || '',
        ...
        logistics: [
          ...
          ['Doors', formatValue(fields['Doors Time'])],
        ...
        deal: [
          ...
          ['EPL Management Fee', formatCurrency(fields['EPL Percent'])],
          ...
          ['Gross Ticket Revenue', formatCurrency(fields['Gross Ticket Revenue'])],
          ['Actual Attendance', formatValue(fields['Actual Attendance'])],
        ],
        people: [
        ...
        notes: [
          ...
          ['Advance Notes', formatValue(fields['Advance Notes'])],
          ['Settlement Notes', formatValue(fields['Settlement Notes'])],
          ['Show Recap', formatValue(fields['Show Recap'])],
          ['Lessons Learned', formatValue(fields['Lessons Learned'])],
        ],
      },
    }
  } catch (error) {
...
async function fetchLinkedRecordList(table, ids = []) {
  return Promise.all(
    [...new Set(ids.filter(Boolean))].map(id => fetchAirtableRecord(table, id))
  )
}

export async function getAdminShowDocument(showId) {
  if (!AIRTABLE_API_TOKEN) {
    return {
      ok: false,
      error: 'Missing Airtable server credential in Vercel.',
  ...
  try {
    const showRecord = await fetchAirtableRecord(TABLES.shows, showId)
    const showFields = showRecord.fields || {}
    const bandIds = linkedIds(showFields.Band)
    const venueId = linkedIds(showFields.Venue)[0]
    const memberIds = linkedIds(showFields['Members Playing'])
    const crewIds = [
      ...linkedIds(showFields['Sound Engineer']),
      ...linkedIds(showFields['Merch Person']),
    ]
    const setlistIds = linkedIds(showFields.SETLISTS)
    ...
    const [
      ...
      fetchLinkedRecordList(TABLES.bands, bandIds),
      venueId ? fetchAirtableRecord(TABLES.venues, venueId) : null,
      fetchLinkedRecordList(TABLES.members, memberIds),
      fetchLinkedRecordList(TABLES.crew, crewIds),
      fetchLinkedRecordList(TABLES.setlists, setlistIds),
      fetchAirtableRecords(TABLES.showSegments).catch(() => []),
      fetchAirtableRecords(TABLES.merchSales).catch(() => []),
    ])

    const segmentRecords = allSegments.filter(record =>
      linkedIds(record.fields?.Show).includes(showId)
    )
    const merchSalesRecords = allMerchSales.filter(record =>
      linkedIds(record.fields?.Show).includes(showId)
    )
    ...
    return {
      ok: true,
      error: null,
      document: buildShowDocumentModel({
        showRecord,
        bandRecords,
        venueRecord,
        memberRecords,
    ...
    return {
      ok: false,
      error: error?.message || 'Unknown show document error.',
      document: null,
    }
  }
}
