...
function airtableRecordUrl(table, recordId) {
  return new URL(`https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(table)}/${recordId}`)
}
...
function normalizeApprovalRecord(record) {
  ...
  return {
    ...
    createdAt: formatDateTime(fields['Created At'] || fields.Created),
...
function normalizeRunRecord(record) {
  ...
  return {
    id: record.id,
    name: fields['Run Name'] || fields['User Request'] || 'Untitled Run',
    agent: formatValue(fields.Agent),
    status: fields['Approval Status'] || fields.Status || '—',
    createdAt: formatDateTime(fields['Created At'] || fields.Created),
    outputSummary: fields['Output Summary'] || '',
  }
}
...
export async function getAdminOpsFoundation() {
  if (!AIRTABLE_API_TOKEN) {
    return {
      ...
      runs: [],
      agents: [],
      counts: { tablesReady: 0, pendingApprovals: 0, recentRuns: 0, activeAgents: 0 },
    }
  }

  const [agentsResult, approvalsResult, runsResult] = await Promise.all([
  ...
  return {
    ok: tables.every(table => table.ready),
    error: tables.find(table => !table.ready)?.error || null,
    tables,
    approvals,
    runs,
    agents,
    ...
    return {
      ...
      show: {
        ...
        logistics: [
          ...
          ['Indoor / Outdoor', formatValue(fields['Indoor/Outdoor'])],
