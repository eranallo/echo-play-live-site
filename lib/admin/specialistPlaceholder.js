...
export async function saveWorkLog({ name, notes, review }) {
  ...
  if (review) {
    ...
    reviewRecord = await createApprovalItem({
      name: `${name} Review`,
      status: 'Pending',
      actionType: payload.kind || 'specialist_review',
      riskLevel: payload.output?.specialist?.risk || 'Medium',
      relatedShow: payload.showId,
      notes,
...
export async function routeWorkAction({ action, source = 'chief_of_staff' }) {
  ...
  const record = await createApprovalItem({
    name: action.label || 'Routed Action',
    status: 'Pending',
    actionType: action.type || 'routed_action',
    riskLevel: action.requiresApproval ? 'High' : 'Low',
    relatedShow: action.showId || '',
    notes,
  })
