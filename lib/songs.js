...
import { cleanSongCatalog } from './public/song-catalog.mjs'
...
const bandIdToInfo = Object.fromEntries(Object.values(bands).map((b) => [b.airtableId, b]))
...
function multiSelectValues(raw) {
  ...
  if (Array.isArray(raw))
    return raw.map((v) => (typeof v === 'string' ? v : v?.name)).filter(Boolean)
  return []
}
...
async function fetchAllLiveSongs() {
  ...
  for (const field of Object.values(F)) query.append('fields[]', field)
  const baseUrl = `${tableUrl(TABLES.SONGS)}?${query}`
  let url = baseUrl
  ...
  try {
    ...
    while (url) {
      ...
      url = data.offset
        ? baseUrl + `&offset=${encodeURIComponent(data.offset)}`
        : null
    }
  } catch (err) {
...
export async function getSongChoicesForBand(bandSlug) {
  ...
  const normalized = bandSongs
    .map((rec) => {
      ...
      return {
        id: createHash('sha256').update(rec.id).digest('hex').slice(0, 20),
        title,
        artist,
  ...
  return cleanSongCatalog(normalized)
}

export async function getSongsForBand(bandSlug) {
  const normalized = await getSongChoicesForBand(bandSlug)
  ...
  const enriched = await Promise.all(
    cleanSongCatalog(normalized).map(async (song) => {
