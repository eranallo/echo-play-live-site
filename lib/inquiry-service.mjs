...
export function validateInquiry(body, bands = []) {
  ...
  if (data.attendance && (!/^[1-9]\d{0,6}$/.test(data.attendance) || Number(data.attendance) > 1000000))
    return { error: 'Please enter a whole number for your expected audience, or leave it blank.' }
  return { data }
}
export async function saveInquiry(body, { bands = [], token, url, fetchImpl = fetch } = {}) {
  ...
  if (d.attendance) fields['Expected Attendance'] = Number(d.attendance)
  if (d.budget) fields.Budget = d.budget
  if (d.production) fields['Special Requests'] = [d.message, `Sound, lighting & stage: ${d.production}`].filter(Boolean).join('\n\n')
  try {
    ...
    return {
      status: 200,
      body: { success: true, bookingEmail: band?.bookingEmail || 'eranallo@echoplay.live', reference: `EPL-${createHash('sha256').update(saved.id).digest('hex').slice(0,12).toUpperCase()}`, receivedAt: new Date().toISOString() },
    }
  } catch {
    return {
      status: 502,
