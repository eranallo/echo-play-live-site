...
export async function GET() {
  ...
  if (artistId) {
    try {
      // limit=20 (Spotify's API rejects 50 on this endpoint despite their docs)
      const url = `https://api.spotify.com/v1/artists/${artistId}/albums?include_groups=album&limit=20&market=US`
      const res = await fetch(url, {
        headers: { Authorization: `Bearer ${token}` },
        cache: 'no-store',
      })
